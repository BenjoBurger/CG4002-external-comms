...
def main():
    ...
    try:
        ...
        is_relay_client_connected = Value('b', False)
        is_ai_client_connected = Value('b', False)
        is_mqtt_client_connected = Value('b', False)
        is_mqtt_server_connected = Value('b', False)

        # Create threads
        relay_thread = Process(target=relay_server_process, args=(RELAY_PORT, relay_to_ai_queue, eval_to_relay_queue, is_relay_client_connected))
        ...
        eval_thread = Process(target=eval_client_process, args=(SERVER_ADDR, server_port, action_queue, eval_to_visualiser_queue, eval_to_relay_queue, is_relay_client_connected, is_ai_client_connected, is_mqtt_client_connected, is_mqtt_server_connected))
